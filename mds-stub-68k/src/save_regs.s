...
|
| save_regs.s — m68000 exception register save/restore.
|
| GDB m68k register layout (18 longs, 72 bytes total):
|     [ 0..7 ]  D0..D7
|     [ 8..14]  A0..A6
|     [15]      A7 — captured here as the USP (the user stack pointer at
|                    exception time, since the stub runs in supervisor
|                    after the CPU pushed the short frame onto SSP)
|     [16]      SR (zero-extended to 32 bits, low 16 bits significant)
|     [17]      PC
|
| 68000 short (group-2) exception stack frame for Trace and TRAP:
|     SP+0  SR  (word)
|     SP+2  PC  (long, big-endian)
| Total = 6 bytes. Restore writes them back so RTE pops the new SR/PC.
...
| Caller is responsible for restore + RTE.
|------------------------------------------------------------------------------
mds_save_regs:
    ...
    moveq   #0, %d0
    | After JSR, SP = exception_frame - 4 (return addr pushed by JSR).
    | We need SR.w at +4 and PC.l at +6 relative to SP here.
    move.w  4(%sp), %d0                  | SR from frame
    move.l  %d0, mds_regs+16*4
    move.l  6(%sp), mds_regs+17*4        | PC from frame
    rts
...
| Called from __stub_*_entry just before RTE.
|------------------------------------------------------------------------------
mds_restore_regs:
